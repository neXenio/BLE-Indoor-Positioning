...
public class RssiArmaFilter {
    ...
    private static double DEFAULT_ARMA_FACTOR = 0.001;

    private int rssiPrediction;
    ...
    public void addMeasurement(int rssi, int factor) {
        //use first measurement as initialization
        if (!isInitialized) {
            rssiPrediction = rssi;
            isInitialized = true;
        }
        armaFactor = armaFactor * factor * 0.001;
        rssiPrediction = (int) (rssiPrediction - armaFactor * (rssiPrediction - rssi));
    }
